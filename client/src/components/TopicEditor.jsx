...
export default function TopicEditor({ babKey, topicIndex, topic, onSave, onClose }) {
  ...
  const insertRedirectButton = () => {
    insertText('<[redirect]>{', '}</[redirect]>')
  }

  const insertDefineBlock = () => {
    const name = prompt('Masukkan nama unik definisi redirect:', 'definisi1')
    if (name && name.trim()) {
  ...
  const insertPreviewTag = () => {
    const name = prompt('Nama unik preview:', 'gambar1')
    if (name && name.trim()) {
  ...
  const renderPreview = () => {
    ...
    html = html.replace(/<\[redirect\]>\s*\{([^}]+)\}\s*<\/\[redirect\]>/g, (match, redirectName) => {
      const escaped = redirectName.replace(/'/g, "\\'")
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-6xl max-h-[90vh] flex flex-col">
        <div className="flex justify-between items-center p-4 border-b">
          <h3 className="text-lg font-bold">Edit Topik: {topic.title}</h3>
